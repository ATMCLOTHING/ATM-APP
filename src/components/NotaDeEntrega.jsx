...
export default function NotaDeEntrega({ supabase, onClose }) {
  ...
  async function buscarCliente(txt) {
    ...
    const {data}=await(esNum?q.or(`codclient.eq.${txt},cedrifclie.ilike.%${txt}%`):q.ilike('nombreclie',`%${txt}%`)).limit(8)
    setCliSugg(data||[])
  }

  function elegirCliente(c){setCliente(c);setCodCli(c.codclient);setCliTxt(c.nombreclie);setCliSugg([])}
  ...
  async function buscarArt(txt,idx){
    ...
    const {data}=await supabase.from('articomp').select('codartic,descartic,talla,preciovent,preciovend,porciva').or(`codartic.ilike.%${txt}%,descartic.ilike.%${txt}%`).limit(10)
  ...
  async function buscarDesc(txt,idx){
    ...
    if(txt.length<2){setArtSugg([]);setArtIdx(null);return}
    const {data}=await supabase.from('articomp').select('codartic,descartic,talla,preciovent,preciovend,porciva').ilike('descartic',`%${txt}%`).limit(10)
    setArtSugg(data||[]);setArtIdx(idx)
  }
  ...
  function quitarLinea(idx){setLineas(prev=>prev.length>1?prev.filter((_,i)=>i!==idx):prev)}
  ...
  async function guardar(){
    ...
    }catch(e){setMsg({tipo:'err',texto:`❌ Error: ${e.message}`})}
    setBusy(false)
  }
    ...
    <div style={P.pagina}>
      ...
      <div style={P.ventana}>
        ...
        <div style={P.titulo}>
          ...
          <div style={P.titNro}>N° <strong>{nroDoc}</strong></div>
        </div>
        ...
        <div style={P.bloque}>
          <div style={P.fila}>
            <Fld label="Cédula / NIT" w={110}>
              <input style={P.inp} value={codCli} onChange={e=>setCodCli(e.target.value)} onBlur={()=>cargarCliente(codCli)}/>
            </Fld>
            <Fld label="Nombre / Razón Social" w={320} rel>
              <input style={P.inp} value={cliTxt} onChange={e=>buscarCliente(e.target.value)} placeholder="Buscar por nombre o cédula…"/>
              {cliSugg.length>0&&(
                <ul style={P.drop}>
                  {cliSugg.map(c=>(
          ...
          <div style={P.fila}>
            <Fld label="Fecha" w={140}><input type="date" style={P.inp} value={fecha} onChange={e=>setFecha(e.target.value)}/></Fld>
            ...
            <Fld label="Fecha de Pago" w={140}><input type="date" style={P.inp} value={fechaPago} onChange={e=>setFechaPago(e.target.value)}/></Fld>
            <Fld label="% Descuento" w={100}><input type="number" style={P.inp} value={pDesc} min={0} max={100} onChange={e=>setPDesc(Number(e.target.value))}/></Fld>
            <Fld label="% IVA" w={80}><input type="number" style={P.inp} value={pIva} min={0} max={100} onChange={e=>setPIva(Number(e.target.value))}/></Fld>
          </div>
          <div style={{...P.fila,alignItems:'center'}}>
            {['Mayor','Detal','Vendedor'].map(t=>(
              <label key={t} style={P.radio}><input type="radio" name="tipo" checked={tipoVta===t} onChange={()=>setTipoVta(t)}/>{' '}{t}</label>
            ))}
            <Fld label="Cédula Vendedor" w={130}><input style={P.inp} value={cedVend} onChange={e=>setCedVend(e.target.value)} onBlur={()=>cargarVendedor(cedVend)}/></Fld>
            <Fld label="Nombre Vendedor" w={190}><input style={{...P.inp,...P.ro}} value={vendedor?.nomvended||''} readOnly/></Fld>
            <Fld label="Celular Vendedor" w={130}><input style={{...P.inp,...P.ro}} value={vendedor?.celular||''} readOnly/></Fld>
          </div>
        </div>
                  ...
                  <tr key={i} style={{background:i%2===0?'#fff':'#f8faff'}}>
                    <td style={P.td}>
                      <div style={{position:'relative'}}>
                        <input style={{...P.ci,width:80}} value={l.codartic} onChange={e=>buscarArt(e.target.value,i)} placeholder="Código"/>
                    ...
                    <td style={{...P.td,minWidth:160}}>
                      <input style={{...P.ci,width:'100%'}} value={l.descartic} onChange={e=>buscarDesc(e.target.value,i)} placeholder="Descripción"/>
                    </td>
                    ...
                    <td style={{...P.td,textAlign:'center'}}>{l.codartic&&<button onClick={()=>quitarLinea(i)} style={P.btnX}>✕</button>}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div style={P.footer}>
          ...
          <div style={P.footCol}>
            <div style={P.medios}>
              {MEDIOS.map(m=>(
                <label key={m} style={P.radio}><input type="radio" name="medio" checked={medio===m} onChange={()=>setMedio(m)}/>{' '}{m}</label>
              ))}
            </div>
...
function IBtn({src,onClick,title,disabled}){
  return(
    <button onClick={onClick} title={title} disabled={disabled} style={{background:'#eef2ff',border:'1px solid #c8d5ea',borderRadius:6,padding:3,cursor:disabled?'not-allowed':'pointer',opacity:disabled?0.4:1,display:'flex',alignItems:'center',justifyContent:'center',width:40,height:36}}>
